...
export default function PestFinder() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setResult(res.data)
      if (isTamil) {
        // Backend already returned Tamil — set it directly as taResult
  ...
  // Trigger translation dynamically when language is switched to Tamil and results exist
  useEffect(() => {
    if (isTamil && result && !taResult && !translating) {
      translateResult(result)
    }
  }, [isTamil, result, taResult, translating, translateResult])
